package algorithmtype.symmetric.aes.mode.ecb.padding.zeropadding;
...
public class CustomSymmetricAesEcbZeroPaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesEcbZeroPaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesEcbZeroPaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/ECB/NoPadding", bouncyCastleProvider);
