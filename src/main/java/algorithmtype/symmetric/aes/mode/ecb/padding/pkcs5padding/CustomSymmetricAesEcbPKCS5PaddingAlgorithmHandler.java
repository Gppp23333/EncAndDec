package algorithmtype.symmetric.aes.mode.ecb.padding.pkcs5padding;
...
public class CustomSymmetricAesEcbPKCS5PaddingAlgorithmHandler {

    MontoyaApi api;
    Logging logging;
    Boolean isBase64 = true;

    public CustomSymmetricAesEcbPKCS5PaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesEcbPKCS5PaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex) {
        try {
            ...
            // 如果是 Base64 编码，先进行解码
            byte[] inputBytes = isBase64
                    ? (isDecrypt == Cipher.DECRYPT_MODE
