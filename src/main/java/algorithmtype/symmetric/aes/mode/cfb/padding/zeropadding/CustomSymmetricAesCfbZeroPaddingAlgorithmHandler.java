package algorithmtype.symmetric.aes.mode.cfb.padding.zeropadding;
...
public class CustomSymmetricAesCfbZeroPaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesCfbZeroPaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesCfbZeroPaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex, String ivHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/CFB/NoPadding", bouncyCastleProvider);
