package algorithmtype.symmetric.aes.mode.cfb.padding.pkcs7padding;
...
public class CustomSymmetricAesCfbPKCS7PaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesCfbPKCS7PaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesCfbPKCS7PaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex, String ivHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/CFB/PKCS7Padding", bouncyCastleProvider);
