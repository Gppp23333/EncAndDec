package algorithmtype.symmetric.aes.mode.ofb.padding.zeropadding;
...
public class CustomSymmetricAesOfbZeroPaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesOfbZeroPaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesOfbZeroPaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex, String ivHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/OFB/NoPadding", bouncyCastleProvider);
