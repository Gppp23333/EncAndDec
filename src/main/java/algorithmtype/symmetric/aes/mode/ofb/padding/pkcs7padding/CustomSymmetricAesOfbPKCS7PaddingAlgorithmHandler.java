package algorithmtype.symmetric.aes.mode.ofb.padding.pkcs7padding;
...
public class CustomSymmetricAesOfbPKCS7PaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesOfbPKCS7PaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesOfbPKCS7PaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex, String ivHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/OFB/PKCS7Padding", bouncyCastleProvider);
