package algorithmtype.symmetric.aes.mode.ofb.padding.pkcs5padding;
...
public class CustomSymmetricAesOfbPKCS5PaddingAlgorithmHandler {

    MontoyaApi api;
    Logging logging;
    Boolean isBase64 = true;

    public CustomSymmetricAesOfbPKCS5PaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesOfbPKCS5PaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
