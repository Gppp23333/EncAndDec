package algorithmtype.symmetric.aes.mode.ctr.padding.zeropadding;
...
public class CustomSymmetricAesCtrZeroPaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesCtrZeroPaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesCtrZeroPaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex, String ivHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/CTR/NoPadding", bouncyCastleProvider);
