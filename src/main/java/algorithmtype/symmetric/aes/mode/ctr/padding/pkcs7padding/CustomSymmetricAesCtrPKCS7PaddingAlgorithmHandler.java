package algorithmtype.symmetric.aes.mode.ctr.padding.pkcs7padding;
...
public class CustomSymmetricAesCtrPKCS7PaddingAlgorithmHandler {
    ...
    public CustomSymmetricAesCtrPKCS7PaddingAlgorithmHandler(MontoyaApi api) {
        this.api = api;
        this.logging = api.logging();
    }

    public byte[] SymmetricAesCtrPKCS7PaddingAlgorithmHandler(Boolean isDecrypt, byte[] selectedBytes, ArrayList<String> paramHexValueList) {
        byte[] handlerBytes;
        if (isDecrypt) {
    ...
    public byte[] DecryptEncryptHandler(int isDecrypt, byte[] data, String keyHex, String ivHex) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("AES/CTR/PKCS7Padding", bouncyCastleProvider);
